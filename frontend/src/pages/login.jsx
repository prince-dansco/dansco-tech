...
export default function Register() {
  ...
  const handleGoogleSignUp = async () => {
    try {
      // Google signup logic will go here
    } catch (err) {
      // setError(err.message);
      console.log(err, 'error in the google button');
  ...
  return (
    <div
      className="max-w-md mx-auto p-6 py-3 px-4 bg-gradient-to-r from-green-800 to-emerald-900 text-white 
              font-bold rounded-lg shadow-lg my-24"
    >
      <h2 className="text-2xl font-bold text-center mb-6 text-white">Login</h2>

      {/* {error && <div className="mb-4 p-3  text-red-700 rounded">{error}</div>} */}
